...
사용자 질문에 대해 **내부 문서(RAG)**와 **웹 검색**을 조합하여 최적의 답변을 제공하는 AI 에이전트 시스템입니다.

### 핵심 가치
- **ReAct 패턴**: 생각(Think) → 행동(Act) → 관찰(Observe) 사이클로 추론 과정 투명화
- **멀티 소스**: 벡터 DB + 웹 검색 + LLM 지식의 하이브리드 활용
...
└───────────────────────────────────────────────────────┼──────────────┘
                    ...
                    │                  │    │                  │    │                  │
                    │ 생각 과정 기록    │    │ • OpenAI Embed   │    │ • DuckDuckGo     │
                    │ (추론 투명화)     │    │ • Milvus Search  │    │ • 웹 검색        │
                    └──────────────────┘    └──────────────────┘    └──────────────────┘
                                                      │                         │
                                                      ▼                         ▼
                                            ┌──────────────────┐    ┌──────────────────┐
                                            │  Milvus/Zilliz   │    │   DuckDuckGo     │
                                            │   Vector DB      │    │      API         │
                                            └──────────────────┘    └──────────────────┘
```
...
6.                          ├→ supervisor 노드: LLM 호출
                            │     │
7.                          │     └→ tool_calls 발생 (think, arag_search 등)
                            │
8.                          └→ tools 노드: 도구 실행
                                  │
9.                                └→ 반복... → 최종 AIMessage
